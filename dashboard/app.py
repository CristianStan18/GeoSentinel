"""
Dashboard MineGuard — interfața pentru inspectori.

Rulează cu: streamlit run dashboard/app.py
"""
...
# Adăugăm rădăcina la path ca să putem importa modulele
...
# Importul LLM este opțional — proiectul rulează 100% offline fără el
try:
...
def build_map(alerts):
    ...
    for alert in alerts:
        ...
        popup_html = (
            ...
            f"Severitate: {SEVERITY_LABELS[alert['overall_severity']]}<br>"
...
def plot_timeseries(site_id: str):
    ...
    fig = make_subplots(
        ...
        subplot_titles=(
            "Subsidență InSAR (deplasare cumulată, mm)",
            "Microseismicitate (magnitudine)",
            "Infiltrație în mină (l/h)",
        ),
    ...
    fig.add_trace(
        go.Scatter(
            ...
            mode="lines+markers", name="Deplasare",
...
st.set_page_config(page_title="MineGuard", layout="wide", page_icon="⛏️")
...
st.markdown('<p class="main-title">⛏️ MineGuard</p>', unsafe_allow_html=True)
st.markdown(
    '<p class="subtitle">Sistem de avertizare timpurie pentru stabilitatea minelor — '
    'fuziune InSAR + seismicitate + hidrologie + AI agent</p>',
    unsafe_allow_html=True,
)
...
try:
    ...
    st.error(
        "Nu există fișier de alerte. Rulează:\n"
        "```\npython -m ingestion.real_data    # SAU ingestion.synthetic_data\n"
        "python -m detection.detect\n```"
    )
    st.stop()

# Verifică sursa datelor uitându-te la coloanele din primul CSV hidro
first_site = alerts[0]["site_id"]
hydro_df = pd.read_csv(DATA_DIR / f"{first_site}_hydro.csv", nrows=1)
data_source_badge = (
    "🌐 Date hibride (Open-Meteo real + InSAR sintetic)"
...
col1.metric("Site-uri monitorizate", len(alerts))
col2.metric("⚠️ Warnings", counts["warning"])
col3.metric("🚨 Alarme", counts["alarm"])
col4.metric("👁️ Watch", counts["watch"])

st.divider()

# ─── Layout principal: hartă | detalii
left, right = st.columns([1, 1])

with left:
    st.subheader("Hartă status site-uri")
    map_obj = build_map(alerts)
    map_data = st_folium(map_obj, width=None, height=500, key="map")

    # Selecție site din click pe hartă sau din selectbox
    site_names = {a["site_id"]: a["site_name"] for a in alerts}
    selected_id = st.selectbox(
        "Sau alege site din listă:",
...
with right:
    ...
    st.markdown(
        ...
        f"Scor anomalie ML: {alert['ml_anomaly_score']:.2f}"
        f"</div>",
        unsafe_allow_html=True,
    )
...
# ─── Serii temporale
st.subheader("📈 Serii temporale fuzionate")
st.plotly_chart(plot_timeseries(selected_id), use_container_width=True)

st.divider()

# ─── Agent / Generator brief
st.subheader("🤖 Briefing operațional MineGuard")
...
with col_b:
    ...
    if LLM_AVAILABLE:
        use_llm = st.toggle(
            "Folosește Claude LLM",
            value=False,
            help="Necesită ANTHROPIC_API_KEY valid în .env. Off = brief generat offline."
        )
    regenerate = st.button("🔄 Generează brief", type="primary")

if regenerate or not cached_path.exists():
    with st.spinner("Generez briefing-ul..."):
        try:
            if use_llm:
            ...
            st.error(
                ...
                "Folosesc varianta offline ca fallback."
            )
            brief_text = generate_brief_offline(selected_id)
            cached_path.write_text(brief_text, encoding="utf-8")
else:
...
st.divider()
st.caption(
    "⚠️ MineGuard este un sistem de suport decizional. Toate alertele trebuie "
    "validate de inspectori autorizați ITM/ANRM înainte de acțiune operațională."
)
